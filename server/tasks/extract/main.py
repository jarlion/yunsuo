...
def extract(source:str) -> Path|None:
    source_path = Path(source)
    # 解压zip
    result = try_unzip(source_path)
    if result:
        return result
    # 解压rar
    result = try_unrar(source_path)
    if result:
        return result
    # 解压7z
    result = try_un7z(source_path)
    if result:
        return result
    return None

def try_unrar(path:Path) -> Path|None:
    try:
        with rarfile.RarFile(path, 'r') as rar_ref:
            rar_ref.extractall(path.parent)
        path = path.parent / path.stem
...
def try_un7z(path:Path) -> Path|None:
    try:
